...
@dataclass
class Entry:
    digits: list[SegmentCombo]
    output: list[SegmentCombo]

def create_entry(instring: str) -> Entry:
    parts = instring.strip().split(" | ")
    digit_strings = parts[0].split(" ")
    digits = [SegmentCombo(list(charac)) for charac in digit_strings]
    output_strings = parts[1].split(" ")
    output = [SegmentCombo(list(charac)) for charac in output_strings]
    return Entry(digits, output)


entries = [create_entry(line) for line in lines]

def criterion(segment_combo: SegmentCombo) -> bool:
    l = len(segment_combo.segments)
